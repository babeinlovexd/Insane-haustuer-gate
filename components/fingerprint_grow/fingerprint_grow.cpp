...
void FingerprintGrowComponent::set_security_level(uint8_t level) {
  ESP_LOGI(TAG, "Setting security level to %d", level);
  // 0x05 is the FINGERPRINT_SECURITY_REG_ADDR
  this->data_ = {WRITE_SYS_PARAM, 0x05, level};
  switch (this->send_command_()) {
    case OK:
      ESP_LOGI(TAG, "Security level set successfully");
      break;
    default:
      ESP_LOGE(TAG, "Reader failed to set security level");
      break;
  }
}
