...
SetSecurityLevelAction = fingerprint_grow_ns.class_("SetSecurityLevelAction", automation.Action)
...
@automation.register_action(
    "fingerprint_grow.set_security_level",
    SetSecurityLevelAction,
    cv.maybe_simple_value(
        {
            cv.GenerateID(): cv.use_id(FingerprintGrowComponent),
            cv.Required("level"): cv.templatable(cv.uint8_t),
        },
        key="level",
    ),
    synchronous=True,
)
async def fingerprint_grow_set_security_level_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])

    template_ = await cg.templatable(config["level"], args, cg.uint8)
